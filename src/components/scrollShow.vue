  ...
  <div class="scroll-show-contain">
    <ul class="title-ad" :class="[{anim:animate==true}]">
      <li class="title-li" v-for="(item, index) in list" :key="'ad'+index">
        <div v-if="item.list">
          <span v-for="(subItem, subIndex) in item.list" :key="'sub'+ subIndex" class="ad-list"
            :style="'border-color:' + inhoverColor">
            <a class="ad-details" @mouseover="mouseOver(subItem)" @mouseleave="mouseLeave(subItem)"
              :style="subItem.isActive?adHoverColor:adinHoverColor" :href="subItem.link" target="_blank"
              :class="hasLine?'has-line':'has-no-line'">
              {{subItem.content}}
            </a>
          </span>
        </div>
        <a v-else class="ad-list" :class="hasLine?'has-line':'has-no-line'" @mouseover="mouseOver(item)"
          @mouseleave="mouseLeave(item)" :style="item.isActive?adHoverColor:adinHoverColor" :href="item.link"
          target="_blank">
