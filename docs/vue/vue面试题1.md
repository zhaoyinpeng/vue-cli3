...
- v-html双大括号的方式会将数据解释为纯文本，而非HTML。为了输出真正的HTML，可以用v-html指令。它等同于JS的innerHtml属性。
- v-pre主要用来跳过这个元素和它的子元素编译过程。可以用来显示原始的Mustache标签。跳过大量没有指令的节点加快编译。{{msg}}如果加上v-pre就直接在页面中显示{{msg}}
- v-cloak这个指令是用来保持在元素上直到关联实例结束时进行编译,解决闪烁问题的。
- v-model双向数据绑定；
...
核心：关于VUE双向数据绑定，其核心是 Object.defineProperty()方法,getter/setter。
